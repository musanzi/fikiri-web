...
const EMPTY_CONTACT: CallContactInfo = { name: '', role: '', email: '', phone: '', links: [] };
...
export default class UpdateCall {
  ...
  protected callResource = httpResource<CallDetailsResponse>(() => `/calls/${this.callId}`);

  protected callModel = linkedSignal<CreateCallFormModel>(() => {
  ...
  protected contactInfo = linkedSignal<CallContactInfo>(() => {
    const contact = this.callResource.hasValue() ? this.callResource.value().data.contact_form : undefined;
    return contact ?? { ...EMPTY_CONTACT, links: [] };
  });

  protected requirements = linkedSignal<CallRequirement[]>(() => {
  ...
  private normaliseForm(sections: IForm[]): IForm[] | undefined {
    if (!sections.some((section) => section.fields.length > 0)) return undefined;
    ...
    return sections
      ...
      .map((section) => ({
        ...
        fields: section.fields.map((field) => ({
          ...
          options: field.options?.map((option) => ({ label: option.label.trim(), value: option.value }))
        }))
      }));
  }
